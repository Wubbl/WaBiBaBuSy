...
**Status:** Most historical issues RESOLVED. See below for current open items.

**⚠️ NOTE:** This file contains both historical issues (mostly resolved) and current items.
All P0 tasks from 2026.01_TODO_ACTIVE.md are COMPLETE as of 2026-02-26.
...
- **Video placeholder frames** - VideoWallpaperRenderer still uses placeholder frames in D2D mode (MVP limitation).

## Historical Issues (Resolved)

---

## 🔴 CRITICAL ISSUE #5: Direct2D Composition - Laggy Animation, No Looping, High CPU (2026-01-03)

**NOTE:** Issue #5 is related to current TASK-008, TASK-009, TASK-010 in TODO_ACTIVE.md.
The issues documented here were observed before LibVLC memory callbacks were implemented.
Current status tracked in TODO_ACTIVE.md.

**Priority:** CRITICAL
**Status:** ROOT CAUSE IDENTIFIED - Architectural Change Required
**Date Reported:** 2026-01-03
**Component:** D2DCompositionService, GifWallpaperRenderer

### Problems Identified

**Issue 5a: GIF Animation is Laggy**
- **Symptom:** GIF animation plays at wrong speed, appears stuttery
- **Root Cause:** Hardcoded 30 FPS in D2DCompositionService (line 28: `_targetFps = 30`)
- **Details:**
  - GIF files have variable frame delays extracted from metadata (correctly implemented in GifWallpaperRenderer.cs:290-338)
  - Example: GIF may have frames at 100ms, 50ms, 200ms delays
  - D2DCompositionService forces 33.33ms frame interval (30 FPS) regardless of GIF timing
  - Result: GIF plays at wrong speed, loses natural timing
- **Files Affected:**
  - `WaBiBaBuSy.WallpaperEngine/Composition/D2DCompositionService.cs:28` - Hardcoded FPS
  - `WaBiBaBuSy.WallpaperEngine/Composition/D2DCompositionService.cs:189` - Frame interval calculation
  - `WaBiBaBuSy.WallpaperEngine/Renderers/GifWallpaperRenderer.cs:290-338` - Correct frame delay extraction (not being used!)

**Issue 5b: Animation Stops Instead of Looping**
- **Symptom:** GIF animation plays once then stops, does not loop
- **Root Cause:** Bug in GifWallpaperRenderer.GetFrameAtPosition() loop logic (line 262-274)
- **Details:**
  - Current code checks `if (frameIndex >= _frameCount) frameIndex = 0;` AFTER the loop
  - But the loop breaks when `timestampMs < accumulatedMs`, so frameIndex never reaches _frameCount
  - When timestampMs exceeds total animation duration, the loop exits with frameIndex at last frame
  - Animation gets stuck on last frame, never loops back to start
- **Fix Required:**
  - Need to calculate `timestampMs % totalDurationMs` BEFORE frame selection
  - Or detect when accumulated time exceeds timestamp and wrap around
- **Files Affected:**
  - `WaBiBaBuSy.WallpaperEngine/Renderers/GifWallpaperRenderer.cs:251-288` - GetFrameAtPosition() method

**Issue 5c: High CPU Usage in UI Process (~10%)**
- **Symptom:** WaBiBaBuSy.UI process uses ~10% CPU during animation playback
- **Root Cause:** Excessive work in main process for every frame (30 FPS):
  1. GifWallpaperRenderer reads frame from GIF file
  2. AnimationLayerRenderer composes frame
  3. CompositionRenderer composites background + animation (GDI+ bitmap operations)
  4. JPEG encoding at 90% quality (CPU intensive)
  5. Base64 encoding for IPC transfer
  6. Send via ProcessCommunicator to D2DPlayer
  7. D2DPlayer receives, deserializes, decodes JPEG, displays
- **Performance Analysis:**
  - 30 FPS × (GIF decode + composition + JPEG encode + base64 encode + IPC) = High CPU
  - Unnecessary work: composing frames in main process only to send them to player
  - Similar to Issue #3 (server sending frames to clients) - already solved with metadata distribution
- **Current Architecture (INEFFICIENT):**
  ```
  Main Process (WaBiBaBuSy.UI):
    ├─ Load GIF file
    ├─ Read frame at position (every 33ms)
    ├─ Compose with background
    ├─ Encode to JPEG (90% quality)
    ├─ Base64 encode
    ├─ Send via IPC ──────────────┐
                                   │
  D2DPlayer Process:               │
  ...
  Total CPU: ~10% in main process + unknown in player
  ```

### Proposed Solution: Metadata-Based IPC (Like Server→Client Architecture)

**User's Suggestion:** "Should we do the same between main process and D2DPlayer? Not submit each frame but submit the animation info/gif or video and the composition system is located in the D2DPlayer?"

**This is EXACTLY the right approach!** The distributed animation system (Phases 1-4) already solved this problem for server→client communication:
- Server sends animation metadata (file path, background config, timing info)
- Clients load animation locally and compose frames themselves
- Result: Server CPU from 80% → <5%, bandwidth from 9 MB/s → <1 MB/s

**Same Architecture Should Apply to Main Process → D2DPlayer IPC:**

```
Proposed Architecture (EFFICIENT):
├─ Main Process (WaBiBaBuSy.UI):
│  ├─ Send IPC command ONCE: "LOAD_ANIMATION"
│  │  ├─ Animation file path (or file data)
│  │  ├─ Background config (color/gradient/image)
│  │  ├─ Animation config (speed, alignment, size)
│  │  └─ Timing info (start time, FPS)
│  └─ No per-frame work!
│
└─ D2DPlayer Process:
   ...
   ├─ Render loop (at GIF's natural frame rate!):
   │  ├─ Get frame from GIF (respecting GIF delays)
   │  ├─ Compose with background
   │  └─ Display directly (no JPEG encoding!)
   └─ Total CPU: Minimal (just local rendering)

Main Process CPU: ~0% (no per-frame work)
Player Process CPU: <5% (GPU-accelerated rendering)
IPC Overhead: 0% (no per-frame messages)
```

### Implementation Plan

**Phase 1: Define New IPC Protocol** (2 hours)
- Add new message types to `WaBiBaBuSy.Player.Common/Messages/`:
  - `PlayerCommandLoadAnimation` - Animation file path + composition config
  - `PlayerCommandStartAnimation` - Timing info (start timestamp, speed)
  - `PlayerCommandStopAnimation` - Stop playback
- Similar to existing distributed animation messages (AnimationMetadata, AnimationStart)

**Phase 2: Move Composition System to D2DPlayer** (4-6 hours)
- Move `AnimationLayerRenderer`, `BackgroundLayerRenderer`, `CompositionRenderer` to D2DPlayer project
- Or create shared library: `WaBiBaBuSy.Composition.Common` (better for code reuse)
- D2DPlayer becomes self-contained composition engine
- Receives metadata, handles all rendering locally

**Phase 3: Update D2DCompositionService** (2-3 hours)
- Remove render loop (lines 187-267)
- Send metadata once during InitializeAsync
- No per-frame work
- Just lifecycle management (start/stop/dispose)

**Phase 4: Respect GIF Frame Timing** (1-2 hours)
- Remove hardcoded 30 FPS
- Use GIF's actual frame delays from metadata
- Variable frame rate based on animation type:
  - GIF: Use frame delays from GIF file
  - Video: Use video's natural frame rate
  - Static image: No updates needed

**Phase 5: Fix GIF Looping** (1 hour)
- Fix GetFrameAtPosition() loop logic in GifWallpaperRenderer
- Calculate `timestampMs % totalDurationMs` before frame selection
...
**Performance:**
- ✅ Main process CPU: 10% → ~0% (no per-frame work)
- ✅ IPC overhead: Eliminated (no per-frame messages)
- ✅ JPEG encoding: Eliminated (no cross-process frame transfer)
- ✅ Base64 encoding: Eliminated
- ✅ Player can use GPU-accelerated rendering directly

**Quality:**
- ✅ GIF animations play at correct speed (respect frame delays)
- ✅ No JPEG compression artifacts
- ✅ Direct rendering to screen (no encoding/decoding overhead)

**Architecture:**
- ✅ Consistent with distributed animation system (server→client)
- ✅ D2DPlayer becomes independent composition engine
- ✅ Easier to test and debug (self-contained player)
- ✅ Code reuse: Same composition system for local and distributed modes

**Scalability:**
- ✅ Can support multiple D2DPlayer instances without main process overhead
- ✅ Each player operates independently
- ✅ Main process just orchestrates, doesn't render

### Code Reuse Opportunities

The distributed animation system already implements this architecture:
- `AnimationDistributor` (server) sends metadata to clients
- `ClientAnimationRenderer` (client) renders locally
- `AnimationOrchestrator` manages timing

**Can Reuse:**
- Composition renderer classes (AnimationLayerRenderer, BackgroundLayerRenderer, CompositionRenderer)
- Animation metadata models (AnimationLayerConfig, BackgroundLayerConfig)
- Timing synchronization logic (if needed for multi-screen)

**New Work:**
- IPC message protocol (similar to gRPC messages but for stdin/stdout)
- D2DPlayer render loop (similar to ClientAnimationRenderer)
- File transfer mechanism (if sending animation data instead of file path)

### Files to Modify

**New Files:**
- `WaBiBaBuSy.Player.Common/Messages/PlayerCommandLoadAnimation.cs` - Animation metadata message
- `WaBiBaBuSy.Player.Common/Messages/PlayerCommandStartAnimation.cs` - Timing message
- `WaBiBaBuSy.Player.Common/Messages/PlayerCommandStopAnimation.cs` - Stop message

**Modified Files:**
- `WaBiBaBuSy.WallpaperEngine/Composition/D2DCompositionService.cs` - Remove render loop, send metadata
- `WaBiBaBuSy.Player.D2D/Program.cs` - Add composition rendering logic
- `WaBiBaBuSy.WallpaperEngine/Renderers/GifWallpaperRenderer.cs` - Fix looping bug
- `WaBiBaBuSy.WallpaperEngine/Direct2D/D2DPlayerHost.cs` - Update IPC commands

**Possibly Move/Share:**
- `WaBiBaBuSy.WallpaperEngine/Composition/AnimationLayerRenderer.cs` → Shared library or D2DPlayer
- `WaBiBaBuSy.WallpaperEngine/Composition/BackgroundLayerRenderer.cs` → Shared library or D2DPlayer
- `WaBiBaBuSy.WallpaperEngine/Composition/CompositionRenderer.cs` → Shared library or D2DPlayer

### Testing Plan

**Test 1: GIF Animation Speed**
- Load GIF with known frame delays (e.g., 100ms per frame)
- Verify animation plays at correct speed
- No stuttering or lag

**Test 2: GIF Looping**
- Load GIF animation
- Verify animation loops seamlessly
- Monitor for 60+ seconds to confirm continuous looping

**Test 3: CPU Usage**
- Load GIF animation
- Monitor WaBiBaBuSy.UI process CPU usage
- Should be <1% during playback (was ~10%)
- Monitor WaBiBaBuSy.Player.D2D process CPU usage

**Test 4: Video Playback**
- Load MP4 video
- Verify plays at correct frame rate
- Verify looping works (if configured)

**Test 5: Static Image**
- Load JPG/PNG image
- Verify displays correctly
- CPU should be ~0% (no animation)

### References

- ✅ Distributed Animation System: `.docs/DISTRIBUTED_ANIMATION_SYSTEM.md`
- ✅ Direct2D Architecture: `.docs/DIRECT2D_RENDERING_ARCHITECTURE.md`
- ✅ Issue #3 (Server CPU): Solved with metadata distribution (see below)
- ✅ Phases 1-4 Complete: AnimationDistributor, ClientAnimationRenderer, TimingSynchronizer

### Related Issues

This issue is architecturally similar to **Issue #3** which was already solved:
- **Issue #3 Problem:** Server composing and sending frames to clients → 80% CPU, 9 MB/s bandwidth
- **Issue #3 Solution:** Send metadata to clients, clients render locally → <5% CPU, <1 MB/s bandwidth
- **Issue #5 Problem:** Main process composing and sending frames to D2DPlayer → 10% CPU
- **Issue #5 Solution:** Send metadata to D2DPlayer, player renders locally → ~0% main CPU

**Same architectural pattern, different context!**
