...
**Status:** Implementation Complete (Separate Player Process for Windows 11 24H2+)
**Build:** 0 Errors
...
The Direct2D composition system renders wallpapers (GIFs, videos, static images) by compositing multiple layers (background + animation) into frames, then displaying them via a dedicated rendering window. This document explains the architecture to prevent future mismatches.
...
| DXGI swap chain parented to Progman (same process) | ✅ Works | ❌ Crashes Explorer |
| DXGI swap chain in separate process | ✅ Works | ✅ Works |
...
## Separate Player Process Architecture (Windows 11 24H2+)

> **CRITICAL DISCOVERY (2025-12-27):** On Windows 11 24H2+, parenting a DXGI swap chain window to the desktop (Progman) crashes `explorer.exe` when clicking on desktop icons. The solution is to render in a **separate process**.

### Problem

When a DXGI swap chain window (Direct3D/Direct2D) is parented to Progman:
- Windows 10 / Windows 11 (pre-24H2): Works fine
- Windows 11 24H2+: Clicking desktop icons freezes/crashes explorer.exe

This is due to how Windows 11 24H2+ handles the layered desktop composition with DXGI surfaces.

### Solution: Separate Player Process

The `WaBiBaBuSy.Player.D2D` project is a standalone executable that:
1. Creates a native Win32 window with DXGI swap chain
2. Handles parenting to desktop itself (same-process SetParent works)
3. Communicates via stdin/stdout IPC protocol
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Main Process (WaBiBaBuSy.UI / WaBiBaBuSy.WallpaperEngine)          │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  D2DPlayerHost                                               │    │
│  │  - Spawns player process                                     │    │
│  │  - Sends commands via stdin (PARENT, COLOR, EXIT)            │    │
│  │  - Reads responses via stdout (HWND, READY, ERROR)           │    │
...
│           ▼                                                         │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  WaBiBaBuSy.Player.D2D.exe (Separate Process)                │    │
│  │  - Creates native Win32 window                               │    │
│  │  - Initializes DXGI swap chain + Direct2D                    │    │
│  │  - Outputs HWND on startup                                   │    │
│  │  - Handles PARENT command (SetParent to Progman)             │    │
│  │  - Runs render loop (~60 FPS)                                │    │
...
**stdout (Player → Host):**
- `HWND:<handle>` - Window handle on startup
- `READY` - Command completed successfully
- `ERROR:<message>` - Error occurred

**stdin (Host → Player):**
- `PARENT:<progman_hwnd>,<defview_hwnd>` - Parent to desktop
- `COLOR:RRGGBB` - Fill with solid color (hex)
- `EXIT` - Clean shutdown

**stderr (Player → Host debug):**
- `DEBUG:*` - Debug messages

### Critical Implementation Details

1. **SetParent FIRST, then SetWindowPos:**
   ```csharp
   // FIRST: SetParent to make us a sibling of DefView
   SetParent(_hwnd, progmanHwnd);

   // THEN: Position behind DefView (now that we're siblings)
   SetWindowPos(_hwnd, defViewHwnd, 0, 0, width, height, SWP_NOACTIVATE);
   ```

2. **No explicit WS_CHILD needed** - SetParent adds it automatically

3. **WS_EX_LAYERED + SetLayeredWindowAttributes(0xFF)** - For mouse click pass-through to desktop icons. Do NOT use `WS_EX_TRANSPARENT` — it crashes `explorer.exe` on Windows 11 24H2+ (fixed 2026-02-05)

4. **Message pump with limit** - Process max 100 messages per frame to avoid infinite loops:
   ```csharp
   int msgCount = 0;
...
5. **PARENT command on main thread** - Must be processed on the window's owning thread (render loop thread), not a background command thread

### Supported Formats in Player.D2D — Added 2026-03-30

`HandleLoadAnimationCommand` in `Program.cs` dispatches on file extension. Each format has its own rendering path and initialization flag:

| Format | Rendering Path | Init Flag | Notes |
|--------|---------------|-----------|-------|
| `.gif` | `ExtractGifFramesToD2D()` — Magick.NET frame extraction → `ID2D1Bitmap[]` GPU cache | `_useNativeD2DComposition = true` | Frame timing from per-frame delays; SpeedMultiplier applied |
| `.mp4` `.avi` `.mkv` `.mov` `.wmv` `.webm` `.flv` | `InitializeNativeVideo()` — LibVLC → raw buffer → `CopyFromMemory` → `ID2D1Bitmap` | `_useNativeD2DVideo = true` | Frame bitmap updated each LibVLC callback |
| `.jpg` `.jpeg` `.png` `.bmp` | Magick.NET single-frame load → `ID2D1Bitmap[0]` — reuses GIF pipeline | `_useNativeD2DComposition = true` | Always renders frame 0; static display |

**Render loop branching** (in `RenderLoop()`):
```
shouldComposeNative = (_useNativeD2DComposition || _useNativeD2DVideo) && _isPlaying

if shouldComposeNative && _d2dGifFrames != null   → GIF / static image path
else if shouldComposeNative && _useNativeD2DVideo  → video path
else if _compositionInitialized                    → legacy CompositionRenderer fallback
```

**If a format is not handled**, neither flag gets set and `HandleStartAnimationCommand` throws `"No composition initialized"` — the player window stays blank with no visible error in the UI.

### Distribution Mode (Sequential vs Simultaneous) — Added 2026-03-20

The `D2DCompositionService.InitializeAsync()` accepts a `perMonitorMode` flag that controls how the virtual canvas is presented to each player:

| Mode | `perMonitorMode` | `VirtualCanvasWidth` | `MonitorOffsetX` | Effect |
|------|-------------------|----------------------|-------------------|--------|
| **Sequential** (spanning) | `false` | Total width of all monitors | This monitor's X offset | Animation moves across entire canvas, appearing on monitors in sequence |
| **Simultaneous** (per-monitor) | `true` | This monitor's width only | `0` | Each monitor renders animation independently, centered on its own screen |

**Key formula in Player.D2D:**
```csharp
var (vx, vy) = MovementCalculator.Calculate(config, elapsedMs, animW, animH, virtualCanvasWidth, height);
_animX = vx - _monitorOffsetX;  // Convert virtual coords → local monitor coords
```

When `perMonitorMode=true`, `virtualCanvasWidth=monitorWidth` and `monitorOffsetX=0`, so each player computes positions relative to its own screen only.

**Data flow:**
```
CrossScreenConfig.DistributionMode (UI setting)
  → MainWindowViewModel: perMonitor = (mode == Simultaneous)
    → D2DCompositionService.InitializeAsync(perMonitorMode)
      → PlayerCommandLoadAnimation { VirtualCanvasWidth, MonitorOffsetX }
        → Player.D2D: MovementCalculator + offset subtraction
```
...
| File | Purpose |
|------|---------|
| `WaBiBaBuSy.Player.D2D/Program.cs` | Separate player process (DXGI rendering) |
| `WaBiBaBuSy.WallpaperEngine/Direct2D/D2DPlayerHost.cs` | Host class to spawn and manage player |
| `WaBiBaBuSy.WallpaperEngine/Composition/D2DCompositionService.cs` | Orchestrates player initialization with canvas layout |
| `WaBiBaBuSy.Models/Wallpaper/MovementCalculator.cs` | Deterministic position math (Static, Linear, Bounce, SineWave, Circular, RandomWalk) |
| `WaBiBaBuSy.Player.Common/Messages/PlayerCommandLoadAnimation.cs` | IPC message with canvas metadata |
| `WaBiBaBuSy.D2DTest/Program.cs` | Test harness for player integration |
